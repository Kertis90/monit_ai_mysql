...
логин набирать не нужно.

Поиску нужна **сервисная учётка** `LDAP_SEARCH_USER`. Пользовательский bind
здесь не подходит принципиально: доступ выдаётся **до** первого входа
человека, и его пароля у нас ещё нет.
...
Прав достаточно самых обычных — чтение каталога, ничего писать не нужно.

Если учётка не задана, строка поиска остаётся на месте, но неактивна и прямо
называет причину. Раньше блок поиска в этом случае просто исчезал, и
администратор видел пустой список без единой подсказки. Выдать доступ по
логину вручную можно всегда — форма под поиском, она же нужна для SSO и OIDC.
