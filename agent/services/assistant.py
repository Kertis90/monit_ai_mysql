...
def tool_specs() -> list:
    ...
    return [
        ...
        {"type": "function", "function": {
            "name": "get_schema",
            "description": "Схема базы из снимка агента: таблицы с "
                           "комментариями из базы, столбцы и индексы "
                           "конкретной таблицы, поиск по именам и по "
                           "комментариям. Нужен, чтобы "
...
# Чем заканчивается сбор данных. Формулировка прямая нарочно: модель,
# которая только что вызывала инструменты, склонна вызывать их дальше.
FINISH_NOTE = (
    "Данные собраны, инструменты закончились. Теперь дай ответ по существу: "
    "числа, имена, факты из полученного выше. Инструменты больше не вызывай "
    "и не описывай, что собираешься сделать, — делать уже нечего, надо "
    "отвечать. Если каких-то данных не хватило, так и скажи и ответь по тем, "
    "что есть.")


# Ответ короче этого ещё может оказаться вступлением. Длинный текст,
# оборванный на двоеточии, — всё-таки ответ, переспрашивать незачем.
PREAMBLE_MAX_CHARS = 400

# Чем кончается вступление, за которым ничего не последовало
PREAMBLE_TAILS = (":", "…", "...", "—", "-", ",")

UNFINISHED_NOTE = (
    "Это не ответ, а вступление: ты написал, что собираешься сделать, и "
    "остановился. Ответь на вопрос по существу — числами, именами и фактами "
    "из данных выше. Инструменты не вызывай и намерений не описывай. Если "
    "данных не хватает, скажи прямо, каких именно.")


def looks_unfinished(text: str) -> bool:
    """Вступление вместо ответа: «сейчас посмотрю…» — и всё.

    Так выглядит ответ, когда модель собиралась вызвать ещё инструмент, а
    звать больше нечего: она объявляет, что сделает, и замолкает. Для
    человека это пустой ответ, сколько бы данных агент ни собрал.
    """
    body = " ".join(str(text or "").split())
    if not body:
        return True
    if len(body) > PREAMBLE_MAX_CHARS:
        return False
...
# Сколько раундов подряд модели позволено не приносить ничего нового,
# прежде чем сбор закончится. Один повтор — обычное дело, два подряд уже
# похожи на топтание на месте.
IDLE_ROUNDS = 2
...
async def llm_with_tools(messages: list, ask=None, note=None,
    ...
    while True:
        ...
        if fresh:
            ...
            # Повтор — ещё не зацикливание. Модель нередко перезапрашивает
            # то же самое, а следующим ходом просит новое: схему таблицы,
            # план запроса. Обрыв на первом же повторе оставлял её с одним
            # инструментом и заставлял отвечать наугад — то есть выдумывать.
            idle += 1
            if idle >= IDLE_ROUNDS:
                logger.info("%d раунда подряд без новых данных — "
                            "заканчиваю сбор", idle)
                break
    ...
    if used:
        convo.append({"role": "user", "content": FINISH_NOTE})
    return convo, used
...
async def build_chat_context(user_message: str, progress=None
    ...
    if cluster:
        ...
        # Про данные спрашивают, не называя города: «напиши запрос по
        # договорам» — это вопрос к схеме, а не к кластеру. Раньше сюда
        # уходили только описания найденных таблиц, а общий список — нет,
        # и модель, не увидев ни одного настоящего имени, сочиняла их.
        listed = False
        for c in clusters:
            try:
            ...
            if not found:
                continue
            # Общий список кладём один раз и говорим, чей он: схемы
            # городов почти одинаковы, но выдавать снимок одного за общий
            # молча нельзя
            if not listed:
                blocks.append("Схема ниже снята с кластера «%s»; в других "
                              "городах она обычно такая же, но проверяй "
                              "имена, если город назван.\n\n%s"
                              % (c["label"], found[0]))
                listed = True
            blocks += found[1:]
